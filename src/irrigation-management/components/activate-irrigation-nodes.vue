<template>
  <div id="activate-nodes">
    ...
    <div class="irrigation-cards">
      <div v-for="(node, index) in irrigationNodes" :key="node.id" class="card">
        ...
        <div class="content">
          <div class="card-info">
            ...
            <p class="indicator-text"><strong>Indicator:</strong> <span style="color: red;">{{ node.indicator }}</span></p>
            <p class="custom-text"><strong>Active Status:</strong> <span :class="node.isActive ? 'status-correct' : 'status-error'">{{ node.isActive ? 'True' : 'False' }}</span></p>
          </div>
...
data() {
  return {
    irrigationNodes: [], // Inicializa un array vacío
    showModal: false,
    currentNode: {},
  };
},
...
methods: {
  async fetchNodes() {
    try {
      ...
      this.irrigationNodes = response.data;
    } catch (error) {
  ...
  activateNode(index) {
    ...
    const interval = setInterval(() => {
      if (node.moisture < 100) {
        node.moisture += 5; // Aumenta el porcentaje de humedad
      } else {
        clearInterval(interval);
        node.isActive = true; // Cambia isActive a true
